...
all_sol = $(addsuffix .pdf,$(instrumentos_clave_de_sol))
all_fa = $(addsuffix .pdf,$(instrumentos_clave_de_fa))
all_livros = $(addsuffix .pdf,$(listlivros))
...
trompaop-Grave.lytex: $(file) Makefile 
	sed -e "s/keepWithTag #'cl/\keepWithTag #'tpaop/g" \
	-e 's/instrumento{InstrCapa}/instrumento{Trompa em Fá - Opcional - Grave}/g' \
	-e 's/instrumento{Original}/instrumento{Trompa em Fá - Opcional - Grave}/g' \
	-e 's/instrumento{Rodape}/instrumento{Trompa Opcional}/g' \
	-e 's/%#escrita#%//g' \
	-e 's/ded-instrumento/ded-trompaop/g' \
	-e 's/jogocl/jogotpaop/g' \
